<template>
  ...
  >
    <p v-if="isLoading" class="p-0 text-sm md:text-xs font-medium md:font-base">
      <svg
        aria-hidden="true"
        role="status"
        class="inline mr-2 w-6 h-6 md:w-5 md:h-5 text-gray-200 animate-spin dark:text-gray-600"
        viewBox="0 0 100 101"
        fill="none"
      ...
      >
        ...
        <path
          ...
          class="fill-teal-500 dark:fill-emerald-400"
        ></path>
      </svg>
      Chargement...
    </p>
    <p v-else class="p-0 text-sm md:text-xs font-medium md:font-base">
      <svg
